    ...
    <div className="h-full w-full flex flex-col overflow-hidden bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white">
      ...
      <div className="w-full max-w-7xl mx-auto p-6 flex-shrink-0">
        ...
        <p className="text-xl text-gray-300">
          Live monitoring of potentially malicious network traffic.
        </p>
      </div>
            ...
            <tbody className="divide-y divide-white/10">
              {networkData.length > 0 ? (
                networkData.map((log, index) => (
                  ...
                  >
                    <td className="w-3/12 px-6 py-4 whitespace-nowrap">
                      <span
                        className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium font-mono border ${isPrivateIP(log.src_ip) ? "bg-green-500/20 text-green-300 border-green-500/30" : "bg-orange-500/20 text-orange-300 border-orange-500/30"}`}
                      >
                        {log.src_ip}
                      </span>
                    </td>
                    <td className="w-3/12 px-6 py-4 whitespace-nowrap">
                      <span
                        className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium font-mono border ${isPrivateIP(log.dest_ip) ? "bg-green-500/20 text-green-300 border-green-500/30" : "bg-orange-500/20 text-orange-300 border-orange-500/30"}`}
                      >
                        {log.dest_ip}
                      </span>
                    </td>
                    ...
                    <td className="w-2/12 px-6 py-4 whitespace-nowrap">
                      <span
                        className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border ${getProtocolInfo(log.protocol).color}`}
                      >
                ...
                <tr>
                  ...
                  >
                    Loading network data or no malicious activity currently
                    detected...
                  </td>
                </tr>
              )}
